...
import InvenItems from './invenItems.js';
export default class ProductInven extends View {
  ...
  `;
    ...
    $inputs;
    $invenContainer;
    #inventory = new InvenItems();
    constructor() {
        super();
        const $content = el(ProductInven.#template);
        this.$form = $content.querySelector('#product-form');
        this.$inputs = Array.from(this.$form.querySelectorAll('input'));
        this.$invenContainer = $content.querySelector('#product-inventory-container');
    ...
    onStoreUpdated({ inventory }) {
        ...
        el(this.$invenContainer, res);
    }
    onSubmit = (e) => {
        e.preventDefault();
        const formDataEntries = new FormData(this.$form).values();
        this.dispatch(Actions.inven_addProduct, {
...
customElements.define('product-inventory', ProductInven);
