import { deepFreeze } from '../util/object.js';

export const VENDING_MACHINE_CONSTANT = deepFreeze({
  PRODUCT: {
    MIN_AMOUNT: 1,
    MIN_PRICE: 100,
    MULTIPLE_PRICE: 10,
  },
  UNIT_INFO: { UNITS: [500, 100, 50, 10], MIN_AMOUNT: 100, MULTIPLE: 10 },
  PURCHASABLE_MONEY: {
    MIN_AMOUNT: 10,
    MULTIPLE: 10,
  },
  MONEY_UNIT: '원',
  AMOUNT_POSTFIX: '개',
});

export const ERROR_MESSAGE = deepFreeze({
  VALIDATION: {
    PRODUCT: {
      NAME_BLANK: '상품명에는 공백을 입력할 수 없습니다',
      PRICE: `상품의 최소 금액은 ${VENDING_MACHINE_CONSTANT.PRODUCT.MIN_PRICE}${VENDING_MACHINE_CONSTANT.MONEY_UNIT} 이상이며 ${VENDING_MACHINE_CONSTANT.PRODUCT.MULTIPLE_PRICE}으로 나누어 떨어져야 합니다`,
      AMOUNT: `상품의 최소 수량은 ${VENDING_MACHINE_CONSTANT.PRODUCT.MIN_AMOUNT}${VENDING_MACHINE_CONSTANT.AMOUNT_POSTFIX} 이상입니다`,
    },
    CHARGE_AMOUNT: `최소 충전 금액은 ${VENDING_MACHINE_CONSTANT.UNIT_INFO.MIN_AMOUNT}${VENDING_MACHINE_CONSTANT.MONEY_UNIT}이며, ${VENDING_MACHINE_CONSTANT.UNIT_INFO.MULTIPLE}으로 나누어 떨어지는 금액으로만 충전할 수 있습니다`,
    SPENDING_MONEY_INPUT: `최소 충전 금액은 ${VENDING_MACHINE_CONSTANT.PURCHASABLE_MONEY.MIN_AMOUNT}${VENDING_MACHINE_CONSTANT.MONEY_UNIT}입니다. ${VENDING_MACHINE_CONSTANT.PURCHASABLE_MONEY.MULTIPLE}으로 나누어 떨어지는 금액으로만 충전할 수 있습니다`,
  },
  SOLD_OUT: '재고가 소진되었습니다. 관리자에게 문의해 주세요',
  NOT_ENOUGH_SPENDING_MONEY: '잔액이 부족합니다. 잔액 충전 후 상품을 선택해 주세요',
});
