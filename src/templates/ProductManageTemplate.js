...
export default state => String.raw`
  <h3>상품 추가하기</h3>
  ...
  <table class="product-inventory">
    ...
    <tbody id="${SELECTOR.PRODUCT_INVENTORY_CONTAINER_ID}">
      ${state.products
        .map(product => productTemplate(product.name, product.price, product.quantity))
        .join('')}
    </tbody>
  </table>
`;
